// Generate random username and password
resource "random_string" "mail_for_good_db_user" {
  length  = 16
  special = false
}

resource "random_password" "mail_for_good_db_pass" {
  length           = 20
  special          = true
  min_special      = 1
  override_special = "~*^+"
}

// Store both in SSM
resource "aws_ssm_parameter" "mail_for_good_db_user" {
  name  = "/${local.environment}/services/mail_for_good/db_user"
  type  = "String"
  value = "mfg_${random_string.mail_for_good_db_user.result}"
}

resource "aws_ssm_parameter" "mail_for_good_db_pass" {
  name  = "/${local.environment}/services/mail_for_good/db_pass"
  type  = "SecureString"
  value = random_password.mail_for_good_db_pass.result
}

// Create postgres role and db
resource "postgresql_role" "mail_for_good" {
  login    = true
  name     = aws_ssm_parameter.mail_for_good_db_user.value
  password = aws_ssm_parameter.mail_for_good_db_pass.value
}

resource "postgresql_database" "mail_for_good" {
  name       = "mail_for_good_${local.environment}"
  owner      = local.master_db_user
  lc_collate = "en_US.UTF-8"
  lc_ctype   = "en_US.UTF-8"
  encoding   = "UTF8"
}

resource "postgresql_default_privileges" "mail_for_good_tables" {
  database    = postgresql_database.mail_for_good.name
  depends_on  = [postgresql_database.mail_for_good, postgresql_role.mail_for_good]
  object_type = "table"
  owner       = local.master_db_user
  privileges  = local.table_privileges
  role        = postgresql_role.mail_for_good.name
  schema      = "public"
}

resource "postgresql_default_privileges" "mail_for_good_sequence" {
  database    = postgresql_database.mail_for_good.name
  depends_on  = [postgresql_database.mail_for_good, postgresql_role.mail_for_good]
  object_type = "sequence"
  owner       = local.master_db_user
  privileges  = local.sequence_privileges
  role        = postgresql_role.mail_for_good.name
  schema      = "public"
}
